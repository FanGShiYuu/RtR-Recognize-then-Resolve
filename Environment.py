...
def intersection_ref_line(entrance, exit):  # check Dgame/画图/交叉口 for
    cv_init = None
    # north entrance
...
def record_ref_line_distance2exit(ref_line):
    ...
    """
    cv = ref_line
    gap_list = np.zeros(len(cv))  # 记录每一个位置点间的距离
...
def find_opponent(vehicle, state_list, front=False):  # opponent是PG中find_opp中计算的结果
    min_dis = 100000
    opponent = None
    ...
    for veh in range(veh_num):
        ...
        while add is not True:
            ...
            for state in state_list:
                ...
                #     if new_state['dis2des'] > state['dis2des'] + 2 * VEH_L:  # 新生成的是后车
                ...
                #     else:
                #         add = False
            new_state['aggressive_intention'] = 0  # 用于判断HV是否存在抢行意图(0无明确意图 1强抢行 -1强让行)
            new_state['heading'] = 0
            new_state['multiple_vehicle'] = 1  # 记录是否形成了batch
...
def write_data(workbook, state_list, t):
    # 值得注意的是 open和save excel只在开始和最后做 因为这个过程如果excel文件大会比较耗时
...
'initialize map'
ALL_REF_LINE, ALL_GAP_LIST, REF_LINE_TOTAL_LENGTH = record_all_possible_ref_line()  # 这个初始化不费时间
